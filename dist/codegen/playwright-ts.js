...
============================================================ */
function buildTSLocatorChain(locators) {
    if (!locators.length)
        return "";
...
function buildTSLocatorArray(locators) {
    if (!locators.length)
        return "[]";
    const items = locators.map((l) => renderPlaywrightTSLocator(l));
    return `[${items.join(", ")}]`;
}
export function generatePlaywrightTS(page, fallback = "chain") {
    const lines = [];
...
//   return chain;
// }
function renderPlaywrightTSLocator(locator) {
    switch (locator.playwrightKind) {
        case "getByRole":
            return `this.page.getByRole("${locator.value}")`;
        case "getByLabel":
...
function buildTSLocatorMap(locators) {
    if (!locators.length)
        return "{}";
