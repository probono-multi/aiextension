...
export function generatePlaywrightPY(page: any, fallback: string = "chain"): string {
  const lines: string[] = [];
...
function renderPlaywrightPythonLocator(locator: any): string {
  switch (locator.playwrightKind) {
    case "getByRole":
      return `self.page.get_by_role("${locator.value}")`;
...
function buildPythonLocatorChain(locators: RankedLocator[]): string {
...
function buildPythonLocatorArray(locators: RankedLocator[]): string {
  if (!locators.length) return "[]";
...
function buildPythonLocatorMap(locators: RankedLocator[]): string {
  if (!locators.length) return "{}";
