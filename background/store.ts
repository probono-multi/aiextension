export function saveElement(descriptor: any) {
  chrome.storage.local.get(["automation_repo"], (res) => {
    const repo: Record<string, any> = res.automation_repo || {};
    ...
    const existingIndex = repo[pageKey].elements.findIndex((e: any) => e.elementId === normalized.elementId);
...
function inferActions(el: any): string[] {
  ...
  if (tag === "button" || tag === "a") actions.push("click");
  if (tag === "input") {
    if (type === "checkbox") actions.push("check");
    else if (type === "file") actions.push("upload");
    else actions.push("fill");
  }

  if (tag === "select") actions.push("select");

  // fallback: clickable by default
  if (!actions.length) actions.push("click");
